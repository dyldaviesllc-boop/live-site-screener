...
function generateRateRefText() {
  const entries = Object.entries(MARKET_RATES)
    ...
    .join(" ");
  return `RATE DATA PRIORITY: If "LIVE MARKET DATA" is provided below a site address, use those real numbers — they come from live APIs (REIT facility scraping, StorTrack, Census, county GIS). Only fall back to these static benchmarks for sites WITHOUT live data.
Static REIT benchmarks (T12 CC $/SF/mo, low\u2013high): ${entries}. T12=15-27% above street.
